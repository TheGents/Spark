...
class ChatRoom extends Component {
  ...
  componentDidMount() {
    ...
    Axios.get(`http://localhost:8080/getmessage/${this.state.roomID}`).then((response)=> {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    Axios.get(`http://localhost:8080/getmessage/${nextProps.navigation.state.params.match.chatRoom}`).then((response)=> {
      this.setState({ kitkats: response.data });
      let katkat = response.data;
      let messageDB = [];
      katkat.map((x)=> {
  ...
  onSend(messages = []) {
    ...
    Axios.post('http://localhost:8080/postmessage', { room_id: room_id, user_id: user_id, created_at: created_at, message: message  } ).then((response)=>console.log(response));
